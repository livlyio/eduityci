...
class Auth extends CI_Controller {
    ...
    function __construct() 
    {
		...
		$this->load->vars('includes_dir', 'http://localhost/includes/');
	...
	/**
	 * Many of the functions within this controller load a custom model called 'flexi_auth_model' that has been created for the purposes of this demo.
	 * The 'flexi_auth_model' file is not part of the flexi auth library, it is included to demonstrate how some of the functions of flexi auth can be used.
	 *
	...
	*/
    ...
    //    print_r($this->input->post()); die();
		...
		if ($this->flexi_auth->ip_login_attempts_exceeded())
		{
			 ...
			 * 
			 * Note: To use this example, you will also need to enable the recaptcha examples in 'models/flexi_auth_model.php', and 'views/login_view.php'.
			*/
			 ...
			 *
			 * Note: To use this example, you will also need to enable the math_captcha examples in 'models/flexi_auth_model.php', and 'views/login_view.php'.
			*/
    ...
    function login_via_ajax()
    {
		if ($this->input->is_ajax_request())
		{
			...
			$this->flexi_auth_model->login_via_ajax();

			die($this->flexi_auth->is_logged_in());
		}
		else
		{
			$this->load->view('login_via_ajax_view', $this->data);
		}
    }
	...
	function register_account()
	{
		...
		$this->load->view('public_examples/register_view', $this->data);
	}
	...
	function resend_activation_token()
	{
		...
		{
			$this->load->model('flexi_auth_model');
			$this->flexi_auth_model->resend_activation_token();
		}
		
		// Get any status message that may have been set.
		$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		

		$this->load->view('public_examples/resend_activation_token_view', $this->data);		
	}
	...
	function forgotten_password()
	{
		...
		$this->load->view('public_examples/forgot_password_view', $this->data);		
	}
	...
	function manual_reset_forgotten_password($user_id = FALSE, $token = FALSE)
	{
		...
		$this->load->view('public_examples/forgot_password_update_view', $this->data);
	}
